...
def set_colors():
    ...
    if mutable_state['animate']:
        color_index = mutable_state['animation_frame']
        if mutable_state['reverse']:
            if mutable_state['animation_frame'] < len(current_colors) - 1:
                mutable_state['animation_frame'] += 1
            else:
                mutable_state['animation_frame'] = 0
        else:
            if mutable_state['animation_frame'] > 0:
                mutable_state['animation_frame'] -= 1
            else:
                mutable_state['animation_frame'] = len(current_colors) - 1
...
def read_pins():
    ...
    if black_button_value == 0 and red_button_value == 0:
        mutable_state['animate_brightness'] = not mutable_state['animate_brightness']
    elif red_button_value == 0 and gray_button_value == 0:
        if mutable_state['animation_slowdown'] < 3:
            mutable_state['animation_slowdown'] += 1
        else:
            mutable_state['animation_slowdown'] = 1
        ...
        mutable_state['animate_brightness'] = False
        update_brightness()
...
while True:
    if read_pins():
        set_colors()
    # Only change colors every animation_slowdown ticks
    if mutable_state['tick'] % mutable_state['animation_slowdown'] == 0:
        if mutable_state['animate_brightness']:
